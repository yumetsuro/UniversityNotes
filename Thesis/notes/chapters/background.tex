...
e.g., identity for regression, softmax or logistic for classification} non linear function such as ReLU, Sigmoid or Tanh.
...
Recurrent Neural Networks (RNNs) \cite{rumelhart1986learning} are a class of neural networks designed to handle sequential values $[\mathbf{x}^1, ..., \mathbf{x}^\tau]$,
 where the order of the data points matters. 
 Unlike FFNs, which process inputs independently thus requiring fixed length data with a separated parameter for each input dimension, 
 also sequential data exhibit temporal dependancies and long context which complexity grows with respect to number of timesteps $\tau$, 
...
$t, k \in \mathbb{N}$. This implies that the same parameter set~$\theta$ governs the dynamics at each time step, 
so that temporal dependencies are captured solely through the hidden state~$\mathbf{h}_t$, without explicit 
dependence on~$t$.
\newline\newline
 Similarly to seminal works as Time Delay Neural Networks (TDNNs) \cite{waibel1989phoneme} and Elman networks \cite{ELMAN1990179}, 
 RNNs maintain a hidden state $\mathbf{h}^t$ that is updated at each time step based on the current input $\mathbf{x}^t$ and the previous hidden state $\mathbf{h}^{t-1}$.
\newline
...
\begin{figure}[H]
    ...
    \node at (6.8,-0.5) (xt1) {$x^{(t-1)}$};
    \node at (8.4,-0.5) (xt) {$x^{(t)}$};
    \node at (10,-0.5) (xt2) {$x^{(t+1)}$};
    \node at (6.8,2.2) (yt1) {$y^{(t-1)}$};
    \node at (8.4,2.2) (yt) {$y^{(t)}$}; 
    \node at (10,2.2) (yt2) {$y^{(t+1)}$}; 
...
to backpropagate the error trough from the end to the start of the unrolled graph \ref{fig:rnn_unrolled} with $O(\tau)$ complexity.
\newline
...
RNNs are the models that most naturally can represent sequential data, however in the last years, they have been largely replaced by Transformer architectures \cite{vaswani2023attentionneed},
...
if we imagine a really long sequence, the gradients will be propagated back through many time steps, this can lead to numerical instability.
...
\newline
Several solutions have been proposed in literature to mitigate this problem, such as Long short-term Memory \cite{article}
 gradient clipping  \cite{pmlr-v28-pascanu13},
residual connections \cite{he2015deepresiduallearningimage}, as well approaches directly acting on the recurrent 
...
\subsubsection{LSTM and Memory}\todo{smetto qui...}
