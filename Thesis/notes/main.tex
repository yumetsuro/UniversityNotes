...
In this thesis, we present a two novel architecture for Reservoir Computing (RC) called Deep Cycle Reservoir (DCR)
and Deep Antisymmetric Reservoir (DAR).
 Both architectures are inspired by recent advances in RC field and use deep layer construction with 
 ...
 outperform on memory capacity and long term prediction tasks the comparing state-of-the-art shallow and deep RC architectures.
 Extensive analysis is carried out to compare the performance of the proposed models with other RC architectures,
 providing insights on the role of modularity and depth in RC.
\end{center}
...
\vfill
...
\chapter{Implementation and Experimental results}\label{ch:experiments}
...
% appendix
\begin{appendices}
\chapter{Jacobian derivation of DCR}\label{ch:appendix}
