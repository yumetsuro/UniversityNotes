...
// Chunked transfer functions to handle large MPI buffers
const size_t MAX_CHUNK_SIZE = 100 * 1024 * 1024; // 100MB chunks

void send_payload_chunked(char* payload_data, size_t total_size, int dest_rank, int tag) {
    if (total_size <= MAX_CHUNK_SIZE) {
        // Small transfer, use regular MPI_Send
        MPI_Send(payload_data, total_size, MPI_BYTE, dest_rank, tag, MPI_COMM_WORLD);
        return;
    }
    
    // Large transfer, use chunked approach
    size_t bytes_sent = 0;
    int chunk_count = (total_size + MAX_CHUNK_SIZE - 1) / MAX_CHUNK_SIZE;
    
    // Send chunk count first
    MPI_Send(&chunk_count, 1, MPI_INT, dest_rank, tag - 1, MPI_COMM_WORLD);
    
    while (bytes_sent < total_size) {
        size_t current_chunk = std::min(MAX_CHUNK_SIZE, total_size - bytes_sent);
        MPI_Send(payload_data + bytes_sent, current_chunk, MPI_BYTE, dest_rank, tag, MPI_COMM_WORLD);
        bytes_sent += current_chunk;
    }
}

void recv_payload_chunked(char* payload_buffer, size_t expected_size, int src_rank, int tag) {
    if (expected_size <= MAX_CHUNK_SIZE) {
        // Small transfer, use regular MPI_Recv
        MPI_Recv(payload_buffer, expected_size, MPI_BYTE, src_rank, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        return;
    }
    
    // Large transfer, use chunked approach
    int chunk_count;
    MPI_Recv(&chunk_count, 1, MPI_INT, src_rank, tag - 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    
    size_t bytes_received = 0;
    for (int i = 0; i < chunk_count; i++) {
        size_t remaining = expected_size - bytes_received;
        size_t chunk_size = std::min(MAX_CHUNK_SIZE, remaining);
        
        MPI_Recv(payload_buffer + bytes_received, chunk_size, MPI_BYTE, src_rank, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        ...
        for (int i = 1; i < num_nodes; i++) {
            ...
            send_payload_chunked(payload_buffer, target_size * g_payload_size, i, 1);
        ...
        recv_payload_chunked(local_payload_block, local_array_size * g_payload_size, 0, 1);
    ...
    if (sequential) {
        ...
        while (current_ranges.size() > 1) {
            ...
            for (size_t i = 0; i < current_ranges.size(); i += 2) {
                // CASE: Carry forward if cannot be merged
    ...
    while (step < num_nodes) {
        ...
        if (DEBUG){
            ...
            if (sender < num_nodes) {
                ...
                recv_payload_chunked(received_payload, received_size * g_payload_size, sender, 4);
            ...
            if (receiver >= 0) {
                ...
                send_payload_chunked(local_payload_block, local_array_size * g_payload_size, receiver, 4);
