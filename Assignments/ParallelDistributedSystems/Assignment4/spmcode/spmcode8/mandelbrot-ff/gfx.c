...
//
// modified to add gfx_draw_row
//

#include <X11/Xlib.h>
#include <X11/Xutil.h>  // added:  needed for XDestroyImage
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void gfx_open( int width, int height, const char *title )
{
	...
	if(visual && visual->c_class==TrueColor) {  // MODIFIED: c_class instead of class to compile the file with a C++ compiler
...
/* Added new function: Draw an entire row of pixels in one call.
   The array 'row' holds 24-bit pixel values for a row of width 'width'.
*/
void gfx_draw_row(int y, int *row, int width) {
    // Allocate memory for the row data (each pixel is a 32-bit integer)
    char *data = (char *)malloc(width * sizeof(int));
    if (!data) return;
    memcpy(data, row, width * sizeof(int));

    // Create an XImage for a single row.
    XImage *image = XCreateImage(gfx_display,
                                 DefaultVisual(gfx_display, 0),
                                 24,           // 24-bit depth
                                 ZPixmap,
                                 0,
                                 data,
    ...
    // Draw the image row at the specified y coordinate
    XPutImage(gfx_display, gfx_window, gfx_gc, image, 0, 0, 0, y, width, 1);
    XFlush(gfx_display);
    
    // XDestroyImage frees both the image structure and the associated data.
    XDestroyImage(image);
}
